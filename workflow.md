...
| ---------- | ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Dev        | `vX.Y.Z-dev-{branch}.{N}` | Автоматически при каждом push в открытый PR в `develop`. Версия вычисляется на основе коммитов: <br/> - `fix:` → patch (X.Y.Z+1)<br/> - `feat:` или `minor:` → minor (X.Y+1.0)<br/> - `major:` или `BREAKING CHANGE` → major (X+1.0.0)<br/> Если целевая версия не изменилась, увеличивается build номер. |
| Beta       | `vX.Y.Z-beta`             | Автоматически при мерже PR в `develop`. Берётся максимальная версия среди всех существующих dev-тегов. Если она выше последней beta, создаётся новый beta-тег, и соответствующие dev-теги удаляются.                                                                                                      |
| Production | `vX.Y.Z`                  | Автоматически при мерже в `main`. Берётся максимальная beta-версия. Если она выше последнего production-тега, создаётся production-тег, и использованный beta-тег удаляется.                                                                                                                              |

`package.json` обновляется автоматически на всех этапах — ручное изменение не требуется.
